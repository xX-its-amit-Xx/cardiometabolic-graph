...
Numbers below come from a local run on the synthetic + MIMIC-IV demo
subsets after `make pipeline`. Re-run `python -m models.evaluate` to
regenerate the full report at [`docs/figures/model_report.md`](docs/figures/model_report.md).

### HbA1c trajectory (regression)

| Model          | Pearson r | MAE (HbA1c %) | Notes                                       |
|----------------|-----------|---------------|---------------------------------------------|
| LightGBM       | **0.72**  | **0.41**      | Tabular baseline on wide feature frame      |
| GAT-GNN (PyG)  | **0.76**  | **0.38**      | Two-layer GATv2, attention-pooled per patient |

### Engagement dropout (binary classification)

| Model          | AUROC     | AUPRC     | Positive rate (test) |
|----------------|-----------|-----------|----------------------|
| LightGBM       | **0.83**  | **0.71**  | ~18%                 |

Explanation outputs live in [`docs/figures/`](docs/figures/):

- `shap_summary.png` — global SHAP beeswarm for the GBM HbA1c model.
...
> **Not yet committed** — these will be generated by running
> `make dashboard` and capturing the patient detail page. The dashboard
> renders deterministic clinician summaries (see
> [`dashboard/_summary.py`](dashboard/_summary.py)) — *no* LLM is in the
> loop so the output stays auditable for regulated environments.

Until then, see the rule index used to generate each summary sentence in
the `RULES` dict at the bottom of `_summary.py`.
